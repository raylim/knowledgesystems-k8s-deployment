...
The release hardening described below is implemented in the working trees and
has passed the compose contract suite. It still requires scoped commits,
immutable image publication, a fresh strict production inventory, and live
cluster credentials before beta promotion.
...
| Compose hydration/release gate | `30be3b7103076e6ebf773e1c5342f0f502cf0b7e` (`codex/reconcile-beta-compose`) | Source revision plus working-tree hardening | Adds versioned v3 staging metadata and row validation before cleanup, shared three-root source policy defaults, inventory checksum/coverage binding, and separate incomplete-asset audit counters. |
...
The tracked backend source now treats Bitly as optional
(`bitly.access.token:`), so a rebuilt image does not depend on an ignored local
properties file.
...
- The dev stack remains healthy and was not restarted or rehydrated during this
  release-hardening pass. The live beta cluster still cannot be mutated from
  this workspace because its Kubernetes context is asking for unavailable
  client credentials.
